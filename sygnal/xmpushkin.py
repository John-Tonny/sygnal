...
QUEUE_TIME_HISTOGRAM = Histogram(
    "sygnal_xm_queue_time", "Time taken waiting for a connection to XMPUSH"
)

SEND_TIME_HISTOGRAM = Histogram(
    "sygnal_xm_request_time", "Time taken to send HTTP request to XMPUSH"
)

PENDING_REQUESTS_GAUGE = Gauge(
    "sygnal_pending_xm_requests", "Number of XMPUSH requests waiting for a connection"
)

ACTIVE_REQUESTS_GAUGE = Gauge(
    "sygnal_active_xm_requests", "Number of XMPUSH requests in flight"
)

RESPONSE_STATUS_CODES_COUNTER = Counter(
    "sygnal_xm_status_codes",
    "Number of HTTP response status codes received from XMPUSH",
    labelnames=["pushkin", "code"],
)

logger = logging.getLogger(__name__)

XMPUSH_URL = b"https://api.xmpush.xiaomi.com/v2/message/all" #regid"
MAX_TRIES = 3
RETRY_DELAY_BASE = 10
...
# though xm-client 'helpfully' extracts these into a separate
# list.
...
class XmPushkin(ConcurrencyLimitedPushkin):
    """
    ...
    def __init__(self, name, sygnal, config):
        super(XmPushkin, self).__init__(name, sygnal, config)
        ...
        # Use the xm_options config dictionary as a foundation for the body;
        # this lets the Sygnal admin choose custom XMPUSH options
        # (e.g. content_available).
        self.base_request_body: dict = self.get_config("xm_options", {})
        if not isinstance(self.base_request_body, dict):
            raise PushkinSetupException(
                "Config field xm_options, if set, must be a dictionary of options"
            )
    ...
    async def _perform_http_request(self, body, headers):
        """
        Perform an HTTP request to the XMPUSH server with the body and headers
        specified.
        Args:
    ...
    async def _request_dispatch(self, n, log, body, headers, pushkeys, span):
        ...
        log.debug("XMPUSH request took %f seconds", time.time() - poke_start_time)
        ...
        if 500 <= response.code < 600:
            ...
            for header_value in response.headers.getRawHeaders(
                ...
                span.log_kv({"event": "xm_retry_after", "retry_after": retry_after})

            raise TemporaryNotificationDispatchException(
                "XMPUSH server error, hopefully temporary.", custom_retry_delay=retry_after
            )
            ...
            try:
                ...
                raise NotificationDispatchException("Invalid JSON response from XMPUSH.")
            if "trace_id" not in resp_object:
                log.error(
            ...
            if len(resp_object["results"]) < len(pushkeys):
                ...
                span.log_kv(
                    {
                        logs.EVENT: "xm_response_mismatch",
            ...
            for i, result in enumerate(resp_object["results"]):
                if "error" in result:
                    ...
                    span.set_tag("xm_error", result["error"])
            ...
            raise NotificationDispatchException(
                f"Unknown XMPUSH response code {response.code}"
            )
    ...
    async def _dispatch_notification_unlimited(self, n, device, context):
        ...
        span_tags = {"xm_num_devices": len(pushkeys)}

        with self.sygnal.tracer.start_span(
            "xm_dispatch", tags=span_tags, child_of=context.opentracing_span
        ) as span_parent:
            data = XmPushkin._build_data(n, device)
            headers = {
                b"User-Agent": ["sygnal"],
            ...
            body["payload"] = urlencode(data)
            body["restricted_package_name"] = "com.blockzero.lianchat.debug"
            body["pass_through"] = 0
            body["title"] = u"链聊"
            body["description"] = u"通知"
            body["notify_type"] = 1
            registration_ids = ""
            for pushkey in pushkeys:
                registration_ids = registration_ids + pushkey + ","
            ...
            for retry_number in range(0, MAX_TRIES):
                ...
                try:
                    ...
                    with self.sygnal.tracer.start_span(
                        "xm_dispatch_try", tags=span_tags, child_of=span_parent
                    ) as span:
            ...
            span_parent.set_tag("xm_num_failed", len(failed))
            return failed
    ...
    def _build_data(n, device):
        ...
        for attr in [
            ...
            if hasattr(n, attr):
                ...
                # body is too long, XMPUSH will reject it.
